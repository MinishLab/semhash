...
Deduplicate on overlapping words instead of meaning with `mode="lexical"`. This uses [MinHash](https://en.wikipedia.org/wiki/MinHash) instead of a model, and is useful for finding copy-pasted or lightly edited texts:
...
texts = [
    ...
    "the master sword can seal the darkness forever",  # Lexical duplicate
...
Scores are estimated Jaccard similarity, and the default threshold is 0.7. To configure the n-grams, pass a `MinHashEncoder` as the model, e.g. `SemHash.from_records(records=texts, model=MinHashEncoder(ngram_size=5, analyzer="char"))`.
...
- **Semantic & Lexical**: SemHash deduplicates on meaning by default, or on overlapping words with `mode="lexical"`.
