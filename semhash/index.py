...
class Index:
    def __init__(self, vectors: np.ndarray, items: list[tuple[dict[str, str], ...]], backend: AbstractBackend) -> None:
        """Make the index."""
        self.items = items
    ...
    def from_vectors_and_items(
        cls, vectors: np.ndarray, items: list[tuple[dict[str, str], ...]], backend_type: Backend
    ) -> Index:
    ...
    def query_threshold(self, vectors: np.ndarray, threshold: float) -> list[list[tuple[dict[str, str], float]]]:
